create database if not exists m3_c0725g1;
use m3_c0725g1;
create table jame(
...
select s.*, ifnull(c.name, 'chưa có lớp')  as class_name, group_concat(p.phone_number) as phone_number
from student s 
join class c on s.class_id = c.id
...
select score, count(score) as sl from student group by score;
-- 9 . Thông kế số lượng học sinh theo điểm và điểm phải lớn hơn 5
select score, count(score) as sl 
from student 
where score >5
group by score;
